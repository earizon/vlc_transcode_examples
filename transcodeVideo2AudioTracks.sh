...
# STEP 1. transcode video. Keep audio tracks
if true ; then
    SOUT="" # Stream Output Transcodage (SOUT) options
    SOUT="${SOUT}#transcode{threads=1,fps=${FPS},vcodec=h264,vb=410,vecn=x264{aud,profile=baseline,crf=30},height=${HEIGHT},width=${WIDTH}}"
    ...
    SOUT="${SOUT}:duplicate{"
        SOUT="${SOUT}dst=std{"
            ...
            # SOUT="${SOUT}dst=streaming/${TS_FORMAT}"
              ...
              SOUT="${SOUT}mux=ts,"
              SOUT="${SOUT}dst=${PORT}_step01.ts,"
            SOUT="${SOUT}}"
    SOUT="${SOUT}}"
fi

# Step 1: transcode video. Keep audio tracks. (--sout-all)
  ${VLC} ${VERBOSITY} ${OPTS} ${ENABLE_AAC} -I dummy ${INPUT} ${QUIT_AFTER_END} --sout-all --sout "${SOUT}" 2>&1 | tee /tmp/l1

# Step 2: Copy video. Split audio tracks. (--sout-ts-pid-audio)
